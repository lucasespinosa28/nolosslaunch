...
export default function Create() {
  ...
  useEffect(() => {
    if (isConfirming) setTransactionStatus('pending');
    if (isConfirmed) setTransactionStatus('success');
    if (isConfirmError) setTransactionStatus('error');
  ...
  return (
    <Container>
      <section className="flex flex-col items-center space-y-8 w-full max-w-[768px] mx-auto bg-gradient-to-b from-gray-800 to-gray-900 p-10 rounded-2xl shadow-2xl">
        ...
        {isConfirmError && <p className="text-red-500 mt-4">Error: {confirmError?.message}</p>}
        {isConfirming && <p className="text-yellow-500 mt-4">Transaction confirming...</p>}
        {transactionStatus === 'success' && (
          <div className="mt-4">
            <p className="text-green-500">Launchpad created successfully!</p>
            {isLatestTokenLoading && <p className="text-yellow-500">Fetching latest token address...</p>}
            {isLatestTokenError && <p className="text-red-500">Error fetching latest token address</p>}
            {latestToken && (
              <p className="text-blue-400 mt-2">
                Latest Token Address: {latestToken}
              </p>
            )}
          </div>
        )}
