...
const BurnToken: React.FC<BuyTokenProps> = ({ symbol, rate, address, contractAddress }) => {
  ...
  const { withdrawal, isDepositing, isDeposited, isPending: isDepositPending, isError: isDepositError, error: depositError } = useWithdrawal();
  ...
  const handleWithdrawal = () => {
    ...
    withdrawal(contractAddress, amount);
  };

  const handleApprove = () => {
    approve(contractAddress, amount,contractAddress);
  };
      ...
      <div className="space-y-4">
        ...
        <div>
          {allowanceLoading ? (
            ...
            <Button onClick={handleWithdrawal} disabled={isDepositing || isDepositPending} className="w-full">
              {isDepositing ? 'Refundyng...' : isDepositPending ? 'Pending...' : `Refund ${symbol}`}
            </Button>
          ) : (
            <Button onClick={handleApprove} disabled={isApproving || isApprovePending} className="w-full">
              {isApproving ? 'Approving...' : isApprovePending ? 'Pending...' : `Approve to burn ${symbol}`}
            </Button>
          )}
        </div>
